...
export async function sendStream(
  ...
  options?: { chunkSize?: number; reply?: Message.TextMessage },
) {
  let fullMessage = "";
  ...
  for await (const textPart of stream) {
    ...
    if (fullMessage.length > chunkBeforeEdit) {
      const messageToUpdate = `${fullMessage.slice(
        currentReplyCount * MAX_MESSAGE_LENGTH,
        (currentReplyCount + 1) * MAX_MESSAGE_LENGTH,
      )}\n\n-------------\n\n Генерирую текст...`;
